...
- **SRT → Audio**: Paste an SRT subtitle file in the Web UI and get a single WAV whose speech aligns with each subtitle's timeline (pad / speed-up ≤1.25× / cut).
...
After loading a model, open the **📜 SRT → Audio** tab, paste an SRT block list, pick a voice (preset or cloned), and click **Tổng hợp theo SRT**. The pipeline synthesizes each subtitle block, trims edge silence, and fits it to that block's `[start, end]` window — padding short clips, speeding up longer ones by up to 1.25×, and hard-cutting anything that still overflows. The output is a single WAV whose speech is aligned to the SRT timeline.
