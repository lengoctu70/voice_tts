...
from apps.srt_ui_handler import synthesize_srt_handler as _srt_ui_handler
...
def synthesize_srt_handler(srt_text, voice_choice, custom_audio, custom_text,
                           mode_tab, temperature, max_chars_chunk,
                           session_id=None):
    """Gradio click handler for the '📜 SRT → Audio' tab.

    Bridges the testable handler in `apps.srt_ui_handler` with the
    module-level model globals + the shared stop event.
    """
    global tts, current_backbone, model_loaded
    yield from _srt_ui_handler(
        srt_text,
        voice_choice,
        custom_audio,
        custom_text,
        ...
        voice_resolver=resolve_voice_id,
    )
...
<div class="header-box">
    ...
    <div class="model-card-content">
        ...
        with gr.Row(elem_classes="container"):
            ...
            with gr.Column(scale=3):
                with gr.Tabs() as main_input_tabs:
                    ...
                    # --- TAB 3: SRT → AUDIO (single-speaker, timeline-aligned) ---
                    with gr.Tab("📜 SRT → Audio", id="srt_tab") as srt_tab:
                        srt_input = gr.Textbox(
                            label="Nội dung SRT",
                            lines=14,
                            placeholder=(
                                "1\n00:00:00,000 --> 00:00:02,000\n"
                                "Xin chào, đây là ví dụ SRT.\n\n"
                                "2\n00:00:02,500 --> 00:00:04,500\n"
                                "Mỗi khối phụ đề sẽ được tổng hợp riêng và đặt đúng thời điểm.\n"
                            ),
                        )
                        srt_log_md = gr.Markdown(
                            "ℹ️ Mỗi khối được tổng hợp riêng, trim silence ở đầu/cuối, "
                            "sau đó padding (nếu ngắn) hoặc speed-up tối đa 1.25× (nếu dài). "
                            "Vượt 1.25× sẽ bị cắt cuối."
                        )
                        btn_generate_srt = gr.Button(
                            "📜 Tổng hợp theo SRT", variant="primary", interactive=False
                        )
        ...
        load_model_event = btn_load.click(
            fn=load_model,
        ...
        # Reflect final model_loaded state onto the SRT button only after load_model
        # finishes — avoids the race where a parallel listener reads the global
        # mid-load (model_loaded is set False at the top of load_model).
        load_model_event.then(
            fn=lambda: gr.update(interactive=model_loaded),
            outputs=btn_generate_srt,
        )
        ...
        srt_gen_event = btn_generate_srt.click(
            fn=synthesize_srt_handler,
            inputs=[srt_input, voice_select, custom_audio, custom_text,
                    current_mode_state, temperature_slider, max_chars_chunk_slider,
                    session_id_state],
