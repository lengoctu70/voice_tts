...
- **Đồng bộ phụ đề (SRT → Audio)**: Dán nội dung SRT vào Web UI để tạo một file WAV duy nhất khớp timeline của phụ đề (pad / speed-up ≤1.25× / cắt).
...
Sau khi tải model, mở tab **📜 SRT → Audio**, dán nội dung SRT, chọn giọng (preset hoặc clone) rồi bấm **Tổng hợp theo SRT**. Mỗi khối phụ đề được tổng hợp riêng, trim silence ở đầu/cuối và fit vào cửa sổ `[start, end]` của khối — pad nếu ngắn, speed-up tối đa 1.25× nếu hơi dài, cắt cuối nếu vẫn vượt. Kết quả là một file WAV duy nhất, đồng bộ với timeline SRT.
